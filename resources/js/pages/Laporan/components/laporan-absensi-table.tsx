...
export default function TableLaporanAbsensi({
  laporan,
  totalScan,
  statusCounts,
}: Props) {
          ...
          <tbody className="[&_tr:last-child]:border-0">
            {laporan.map((data, index) => (
              ...
              >
                ...
                <td className="p-4">
                  {data.scanned_at ? formatTime(data.scanned_at) : '-'}
                </td>
              </tr>
            ))}
            {laporan.length > 0 && (
              <>
                <tr className="hover:bg-muted/50 border-b font-semibold transition-colors">
                  ...
                  <td className="px-4 py-1.5">
                    Total Hadir : {statusCounts.hadir}
                  </td>
                  <td className="px-4 py-1.5">
                    Total Izin : {statusCounts.izin}
                  </td>
                  <td className="px-4 py-1.5">
                    Total Sakit : {statusCounts.sakit}
                  </td>
                </tr>
                <tr className="hover:bg-muted/50 border-b font-semibold transition-colors">
                  <td colSpan={1}></td>
      ...
      <div className="block lg:hidden">
        {laporan.length > 0 ? (
          <div className="divide-y divide-gray-200">
            ...
            <div className="border-t p-4 text-sm font-semibold text-black">
              Total Absensi: {totalScan}
            </div>
          </div>
        ) : (
