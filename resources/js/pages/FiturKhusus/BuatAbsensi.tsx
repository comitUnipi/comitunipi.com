          ...
          {qrData && qrCodeSvg && (
            <div className="bg-muted mt-10 max-w-md rounded-lg border p-6">
              ...
              <p className={`mb-4 text-sm font-medium ${qrData.is_active ? 'text-green-600' : 'text-red-500'}`}>
                {qrData.is_active ? 'QRCode saat ini aktif dan bisa digunakan untuk absensi.' : 'QR Code ini sudah tidak aktif.'}
              </p>
              ...
              <div className="text-muted-foreground mt-10 space-y-1 text-sm">
                <p>
                  <span className="font-semibold">Nama Kegiatan:</span> {qrData.kegiatan?.name}
                </p>
                <p>
                  <span className="font-semibold">Kegiatan untuk:</span> {capitalizeFirstLetter(qrData.kegiatan?.audiens || '')}
                </p>
                <p>
                  <span className="font-semibold">Waktu:</span> {qrData.start_time} - {qrData.end_time}
                </p>
                <p>
                  <span className="font-semibold">Status:</span> {qrData.is_active ? 'Aktif' : 'Nonaktif'}
                </p>
              </div>
